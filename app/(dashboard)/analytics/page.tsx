...
export default async function WorkspaceAnalyticsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="page-wrap">
    ...
    <section className="analytics-panel workspace-usage-strip"><header><div><p className="eyebrow">Combined AI usage</p><h2>Usage for this date range</h2></div><Coins size={20} /></header><div><span><strong>{usage.inputTokens.toLocaleString()}</strong><small>input tokens</small></span><span><strong>{usage.outputTokens.toLocaleString()}</strong><small>output tokens</small></span><span><strong>{usage.embeddingTokens.toLocaleString()}</strong><small>embedding tokens</small></span><span><strong>${usage.estimatedCost.toFixed(4)}</strong><small>estimated cost</small></span></div></section>
  </main>;
}
