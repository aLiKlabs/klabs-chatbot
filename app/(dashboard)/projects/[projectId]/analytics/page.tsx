...
export default async function AnalyticsPage({ params, searchParams }: { params: Promise<{ projectId: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="page-wrap project-page"><ProjectHeader project={project as Project} active="analytics" />
    ...
    <section className="analytics-panel usage-panel"><header><div><p className="eyebrow">AI usage</p><h2>Tokens, latency, and estimated cost</h2></div><Coins size={20} /></header><div><article><Clock3 size={17} /><span><strong>{latency.toLocaleString()} ms</strong><small>average response latency</small></span></article><article><MessageCircleQuestion size={17} /><span><strong>{usage.inputTokens.toLocaleString()}</strong><small>input tokens</small></span></article><article><Bot size={17} /><span><strong>{usage.outputTokens.toLocaleString()}</strong><small>output tokens</small></span></article><article><Database size={17} /><span><strong>{usage.embeddingTokens.toLocaleString()}</strong><small>embedding tokens</small></span></article><article><Coins size={17} /><span><strong>${usage.estimatedCost.toFixed(4)}</strong><small>estimated API cost</small></span></article></div><p>Cost remains $0 in free local test mode. Production rates are configurable through environment variables.</p></section>
  </main>;
}
